 ...
 * @typedef {import('../../../../src/vscode/web.api.type').WebVscodeApi} WebVscodeApi
 */
/**
 * @template T
 * @typedef {import('../../../../src/vscode/web.api.type').WebVscodeContextApi<T>} WebVscodeContextApi<T>
 */
/**
 * @template T
 * @typedef {import('../../../../src/vscode/web.api.type').WebDataApi<T>} WebDataApi<T>
 */
/**
 * @template A
 * @template R
 * @typedef {import('../../../../src/vscode/web.api.type').WebApiFunction<A, R>} VscodeAsyncFunction
 */
/**
 * Vscode api for web
 * @template T1
...
class Vscode {
    ...
    get _cmd() {
        if (this._api === undefined) {
            const api = { api: {} };
            /**@type {WebDataApi<T1> & WebVscodeApi & WebVscodeContextApi<T2>} */
            // @ts-ignore
            this._api = new Proxy({}, {
                get: (target, property, receiver) => {
                    // @ts-ignore
                    return (data) => this.post({ cmd: property, args: data, reply: true });
                }
            });
        }
        return this._api;
    ...
    /**
     * Get webview data
     * @type {VscodeAsyncFunction<void, T1>}
     */
    getWebviewData = () => {
        return this.post({ cmd: this._cmd.getWebviewData.name });
    }
